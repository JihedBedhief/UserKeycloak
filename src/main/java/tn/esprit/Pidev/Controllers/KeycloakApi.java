...
public class KeycloakApi {
    ...
    public List<String> getUserRoles(String username) {
        ...
        RealmResource realm = keycloak.realm("pidev");
    ...
    public ResponseEntity<String> addRoleToUser(@RequestBody Map<String, String> roleData) {
        ...
        try {
            RoleRepresentation role = keycloak.realm("pidev").roles().get(roleName).toRepresentation();
            UserRepresentation user = keycloak.realm("pidev").users().search(username).get(0);
            keycloak.realm("pidev").users().get(user.getId()).roles().realmLevel().add(Arrays.asList(role));
    ...
    public ResponseEntity<String> deleteRoleFromUser(@RequestBody final Map<String, String> roleData) {
        ...
        try {
            RoleRepresentation role = keycloak.realm("pidev").roles().get(roleName).toRepresentation();
            UserRepresentation user = keycloak.realm("pidev").users().search(username).get(0);
            keycloak.realm("pidev").users().get(user.getId()).roles().realmLevel().remove(Arrays.asList(role));
    ...
    public void addUserPassword(@PathVariable String userId, @RequestBody String password) {
        ...
        keycloak.realm("pidev").users().get(userId).resetPassword(credential);
    }

    @PutMapping("/users/{userId}/disable")
    public void disableUser(@PathVariable String userId) {
        UserResource userResource = keycloak.realm("pidev").users().get(userId);
    ...
    public ResponseEntity<RoleRepresentation> getRoleById(@PathVariable String roleId) {
        try {
            // RoleResource roleResource = keycloak.realm("pidev").rolesById().get(roleId);
            RoleRepresentation role = keycloak.realm("pidev").rolesById().getRole(roleId);
            if (role != null) {
